...
class Level:
    ...
    def loadEntities(self, data):
        try:
            ...
            [self.addPot(x, y, item) for x, y, item in data["level"]["entities"]["pot"]]
        ...
        for x in range(*data["level"]["layers"]["sky"]["x"]):
            layers.append(
                (
                        ...
                        + [
                            Tile(
                                self.sprites.spriteCollection.get("grass" if x < 60 else"ground_boss"),
                                pygame.Rect(x * 32, (y - 1) * 32, 32, 32),
                            )
                            for y in range(*data["level"]["layers"]["grass"]["y"])
                        ]
                        + [
                            Tile(
    ...
    def loadObjects(self, data):
        ...
        for x, y in data["level"]["objects"]["brick"]:
            self.addBrickSprite(x, y)
        for x, y, z in data["level"]["objects"]["pipe"]:
            self.addPipeSprite(x, y, z)
        for x, y in data["level"]["objects"]["grass"]:
            self.level[y][x] = Tile(
                self.sprites.spriteCollection.get("grass"),
    ...
    def addPipeSprite(self, x, y, length=2):
        try:
            ...
            self.level[y + 1][x + 1] = Tile(
                self.sprites.spriteCollection.get("pipe2R"),
                pygame.Rect((x + 1) * 32, (y + 1) * 32, 32, 32),
            )
        except IndexError:
            return
    ...
    def addBrickSprite(self, x, y):
        try:
            self.level[y][x] = Tile(self.sprites.spriteCollection.get("brick"), 
                                    pygame.Rect(x * 32, y * 32, 32, 32),)
        except IndexError:
            return
    
    def addGrassSprite(self, x, y):
        try:
            self.level[y][x] = Tile(
                self.sprites.spriteCollection.get("grass"), None
            )
        except IndexError:
            return
    ...
    def addPotion(self, x, y):
        self.entityList.append(Potion(self.screen, self.sprites.spriteCollection, x, y, self, self.sound))
